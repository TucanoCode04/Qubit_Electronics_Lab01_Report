Using the same RLC circuit as the exercise before, this time we are given the values of the capacitance, the resonance frequency, and the quality factor that we want from the circuit, reported in Table \ref{tab:parameters}.
...
As we can see, the results obtained are close to the theoretical values. A curious thing is that at the beginning the values found weren't so close, because the step in frequency was too big, causing an under-approximation in the calculations. Increasing the number of steps from 10000 to 100000, thus decreasing the size of the step, the approximation obtained was better, as we can see.
